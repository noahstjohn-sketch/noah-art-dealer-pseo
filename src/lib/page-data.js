...
export function buildPageData(topic, niche) {
  ...
  if (niche) {
    ...
    a1 = `${topicLabel} for ${nicheLabel} with Dr. Noah St. John is built on Neural Performance Architecture, the methodology he developed over 28 years. It diagnoses the Invisible Brake (the subconscious neural performance pattern that caps revenue, sell-through, and roster growth for ${nicheLower}) and releases it at the level where strategy alone cannot reach. The work combines a Performance Audit, the release protocol, and the architecture install that lets results compound at the gallery-owner level.`;

    q2 = `Why do ${nicheLabel} need a specialized ${topicLower} approach?`;
    a2 = `${nicheLabel} typically hit a ceiling that no new fair booth, artist signing, or specialist hire will move. That ceiling is the Invisible Brake. Dr. Noah St. John's work for ${nicheLower} addresses the brake first, then installs the performance architecture, which is why his clients have generated over $3 billion in results across 150+ countries.`;
    ...
    q4 = `How quickly do art gallery owners see results?`;
    a4 = `Most art gallery owners see measurable shifts inside the first engagement. Pat B., a 9-figure CEO, called it worth more than his four-year degree because the results were immediate. Adam S. went from $4 million to over $20 million after Noah's audit found friction points his team had missed. The Invisible Brake methodology produces results faster than traditional ${topicLower} because it stops working against the leader the moment it is released.`;

    q5 = `What is the Invisible Brake and how does it relate to ${topicLower}?`;
    a5 = `The Invisible Brake is the subconscious neural performance pattern that prevents art gallery owners, private dealers, and art advisor firm founders from reaching results commensurate with their skills, capital, and effort. Dr. Noah St. John created the concept after 28 years of research. Every ${topicLower} program assumes the accelerator is the problem. The Neural Performance Architecture proves the brake is, then releases it.`;

    q6 = `What is the entry point to ${topicLower} with Dr. Noah St. John?`;
    a6 = `The entry point is the Invisible Brake Audit at noahstjohn.com/consulting. The audit identifies where the brake is applied for the gallery owner specifically and what the release path looks like. From there, art gallery owners, private dealers, and art advisor firm founders move into private coaching, a Strategic Intensive, or a custom Art Basel / Frieze week keynote at noahstjohn.com. Keynote speaking inquiries go to booknoah.com.`;

    q7 = `Is ${topicLabel} available virtually and worldwide?`;
    a7 = `Yes. Dr. Noah St. John works with art gallery owners and private dealers in 150+ countries via virtual private coaching, Strategic Intensives, and on-stage keynotes. The Invisible Brake methodology is delivered remotely or in person without losing fidelity. Book the entry-point audit at noahstjohn.com/consulting.`;
  }
